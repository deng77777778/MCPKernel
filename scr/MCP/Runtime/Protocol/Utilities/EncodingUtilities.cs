...
namespace MCP.Protocol
{
    ...
    internal static class EncodingUtilities
    {
        ...
        public static ReadOnlyMemory<byte> EncodeToBase64Utf8(ReadOnlyMemory<byte> data)
        {
            ...
            System.Diagnostics.Debug.Assert(status == OperationStatus.Done, "Base64 encoding should succeed for valid input data");
            System.Diagnostics.Debug.Assert(bytesWritten == buffer.Length, "Base64 encoding should always produce the same length as the max length");
        ...
        public static ReadOnlyMemory<byte> DecodeFromBase64Utf8(ReadOnlyMemory<byte> base64Data)
        {
            ...
            if (Base64.DecodeFromUtf8(base64Data.Span, buffer, out _, out int bytesWritten) == OperationStatus.Done)
            {
                ...
                System.Diagnostics.Debug.Assert(bytesWritten <= buffer.Length, "Base64 decoding should never produce more bytes than the max length");
                return buffer.AsMemory(0, bytesWritten);
            }
            else
            {
