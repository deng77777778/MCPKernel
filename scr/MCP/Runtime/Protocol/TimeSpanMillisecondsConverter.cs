                ...
                case JsonToken.String:
                    {
                        ...
                        if (long.TryParse(strValue, out long milliseconds))
                        {
                            return FromMillisecondsClamped(milliseconds);
                        }
                            ...
                            out double doubleValue))
                        {
                            ...
                            return FromTicksClamped(doubleValue * TimeSpan.TicksPerMillisecond);
                        }

                        throw new JsonSerializationException(
                            $"Unable to convert string '{strValue}' to TimeSpan.");
                    }
                case JsonToken.Null:
                    return TimeSpan.Zero;
                default:
                    throw new JsonSerializationException(
                        $"Unable to convert {reader.TokenType} to TimeSpan.");
            }
        }
