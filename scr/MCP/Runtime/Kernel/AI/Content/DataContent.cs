    ...
    public class DataContent : AIContent
    {
        ...
        public ReadOnlyMemory<byte> Data
        {
            get
            {
                if (_data is null)
                {
                    UnityEngine.Assertions.Assert.IsNull(_dataUri, "Expected dataUri to be initialized.");
                    _data = _dataUri!.ToByteArray();
                }
        ...
        public ReadOnlyMemory<char> Base64Data
        {
            get
            {
                string uri = Uri;
                int pos = uri.IndexOf(',');
                UnityEngine.Assertions.Assert.IsTrue(pos >= 0, "Expected comma to be present in the URI.");
