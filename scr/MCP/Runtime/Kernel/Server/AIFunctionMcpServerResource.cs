...
namespace MCP.Kernel.Server
{
    ...
    internal sealed class AIFunctionMcpServerResource : McpServerResource
    {
        ...
        private readonly string[] _templateVariableNames = Array.Empty<string>();
            ...
            McpServerResourceCreateOptions? options)
        {
            ...
            return Create(
              AIFunctionFactory.Create(method: method, target: target, CreateAIFunctionFactoryOptions(method, options)),
                options);
        }
        ...
        private static AIFunctionFactoryOptions CreateAIFunctionFactoryOptions(
            ...
            new()
            {
                ...
                {
                    if (pi.ParameterType.IsAugmentedWith<ReadResourceRequestParams>())
                    {
                        return new()
                        {
        ...
        private static readonly ConcurrentDictionary<Type, Func<string, object?>> s_convertersCache = new();
        ...
        public static new AIFunctionMcpServerResource Create(AIFunction function, McpServerResourceCreateOptions? options)
        {
            ...
            return new AIFunctionMcpServerResource(function, resource, options?.Metadata ?? new List<object>());
        }
            ...
            RequestContext<ReadResourceRequestParams> request, CancellationToken cancellationToken = default)
        {
            ...
            return result switch
            {
                ...
                TextContent tc => new()
                {
                    Contents = new List<ResourceContents>() { new TextResourceContents { Uri = request.Params.Uri, MimeType = ProtocolResourceTemplate.MimeType, Text = tc.Text } },
                },

                DataContent dc => new()
                {
                    Contents = new List<ResourceContents>(){new BlobResourceContents
                    {
                ...
                string text => new()
                {
                    Contents = new List<ResourceContents>() { new TextResourceContents { Uri = request.Params.Uri, MimeType = ProtocolResourceTemplate.MimeType, Text = text } },
                },
